...
from integrity_guard_manager import ensure_project_integrity_or_exit, verify_manifest
...
session_info_path = os.path.join(data_dir, "settings", "session_info.json")
update_state_path = os.path.join(data_dir, "settings", "update_state.json")
webhooks_path = os.path.join(data_dir, "settings", "webhook_settings.json")
...
updater_script = os.path.join(parent_dir, "managers", "update_manager.py")
...
version_path = os.path.join(data_dir, "info", "version.txt")

TOOL_REPO = "anonymous98268/VRCWG"
LOGGED_INFO_REPO = "anonymous98268/VRCWG-Logged-Info"
GITHUB_BRANCH_CANDIDATES = ("main", "master")
GITHUB_HEADERS = {
    "User-Agent": "VRCWG/0.4",
...
def ensure_update_state_file():
    if not os.path.isfile(update_state_path):
        save_json(update_state_path, json.loads(json.dumps(UPDATE_STATE_DEFAULT)))


def load_update_state():
    ensure_update_state_file()
    return load_json_safe(update_state_path, UPDATE_STATE_DEFAULT)
...
def parse_version_rank(version_text):
...
def github_raw_candidates(repo_name, rel_path):
...
def github_zip_candidates(repo_name):
    return [
        (branch, f"https://github.com/{repo_name}/archive/refs/heads/{branch}.zip")
        for branch in GITHUB_BRANCH_CANDIDATES
    ]


def fetch_remote_text(urls, timeout=10):
    last_error = ""
    for url in urls:
        try:
            response = requests.get(url, timeout=timeout, headers=GITHUB_HEADERS)
            if response.status_code == 200:
                return response.text, url, ""
            last_error = f"HTTP {response.status_code}"
        except Exception as exc:
            last_error = str(exc)
    return "", "", last_error or "Unknown GitHub error"


def fetch_remote_zip(repo_name, timeout=20):
    last_error = ""
    branch_candidates = list(GITHUB_BRANCH_CANDIDATES)
    try:
        repo_info = fetch_github_json(f"https://api.github.com/repos/{repo_name}", timeout=timeout)
        default_branch = normalize_text(repo_info.get("default_branch", ""))
        if default_branch:
            branch_candidates = [default_branch] + [branch for branch in branch_candidates if branch != default_branch]
    except Exception as exc:
        last_error = str(exc)
    ...
    for branch in branch_candidates:
        ...
        try:
            ...
            last_error = str(exc)
    return b"", "", last_error or "Unknown GitHub error"


def fetch_github_json(url, timeout=15):
    response = requests.get(url, timeout=timeout, headers=GITHUB_HEADERS)
    response.raise_for_status()
    return response.json()


def extract_first_matching_logged_info(zip_bytes):
    try:
    ...
    candidate_names = [
        ...
        if name.lower().endswith(".txt") and "logged_players_" in os.path.basename(name).lower()
    ]
    if not candidate_names:
        return "", b""
    candidate_names.sort()
    chosen_name = candidate_names[-1]
    try:
        return os.path.basename(chosen_name), archive.read(chosen_name)
    except Exception:
        return "", b""


def extract_project_root_from_zip_bytes(zip_bytes):
    try:
        archive = zipfile.ZipFile(io.BytesIO(zip_bytes))
    except Exception:
        return "", None
    for name in archive.namelist():
        if name.endswith("RUN ME.py"):
            root_prefix = name[: -len("RUN ME.py")].rstrip("/\\")
            return root_prefix, archive
    return "", archive


def extract_remote_version_from_zip(zip_bytes):
    root_prefix, archive = extract_project_root_from_zip_bytes(zip_bytes)
    if not archive or not root_prefix:
        return "", ""
    try:
        version_path = f"{root_prefix}/Stuff/_data/info/version.txt"
        version_text = archive.read(version_path).decode("utf-8", errors="replace")
        root_name = os.path.basename(root_prefix)
        return normalize_text(version_text), root_name
    except Exception:
        return "", os.path.basename(root_prefix)
...
def refresh_sound_caches():
    ...
    for entry in reason_entries:
        ...
        if reason and warning and reason not in new_reason_map:
...
def find_sound_profile_for_user(username, user_id):
    ...
    stored_warning = normalize_key(logged_entry.get("Warning", ""))
    if stored_warning in {"red", "orange", "yellow"}:
...
def load_webhook_settings():
    return load_json_safe(webhooks_path, {"webhooks": [""], "join_enabled": True, "leave_enabled": True, "reasons": {}})


def send_reason_webhooks(event_type, username, user_id):
    logged_entry = lookup_logged_user(user_id, username)
    if not logged_entry:
        return

    settings = load_webhook_settings()
    if event_type == "join" and not settings.get("join_enabled", True):
        return
    if event_type == "leave" and not settings.get("leave_enabled", True):
        return

    reasons = split_lookup_values(logged_entry.get("Reasons", ""))
    if not reasons:
        return

    enabled_reasons = settings.get("reasons", {})
    matched_reasons = [reason for reason in reasons if enabled_reasons.get(reason)]
    if not matched_reasons:
        return

    webhooks = [normalize_text(url) for url in settings.get("webhooks", []) if normalize_text(url)]
    if not webhooks:
        return

    message = (
        f"VRCWG {event_type.title()}\n"
        f"User: {username}\n"
        f"UserID: {user_id}\n"
        f"Reason: {', '.join(matched_reasons)}\n"
        f"World: {get_state_snapshot().get('world') or 'Unknown'}"
    )
    for url in webhooks:
        try:
            requests.post(url, json={"content": message}, timeout=8)
        except Exception as exc:
            set_status("manager", "connection", "WG-WEBHOOK-SEND", f"Webhook send failed: {exc}")
...
def append_detected_link(link):
    ...
    entries.append({
        ...
        "detected_at_unix": timestamp,
    })
    cutoff = timestamp - 600.0
    entries = [entry for entry in entries[-300:] if get_link_timestamp(entry, timestamp) >= cutoff]
    with open(links_path, "w", encoding="utf-8") as f:
        json.dump(entries, f, indent=4)


def get_link_timestamp(entry, default_time=None):
    default_time = time.time() if default_time is None else default_time
    try:
        raw = float(entry.get("detected_at_unix", 0.0) or 0.0)
        if raw > 0.0:
            return raw
    except Exception:
        pass
    detected_at = normalize_text(entry.get("detected_at", ""))
    if detected_at:
        try:
            return datetime.strptime(detected_at, "%Y-%m-%d %H:%M:%S").timestamp()
        except Exception:
            pass
    return default_time


def prune_detected_links():
    ensure_links_file()
    entries = load_json_list(links_path)
    if not entries:
        return
    now_ts = time.time()
    filtered = [entry for entry in entries if get_link_timestamp(entry, now_ts) >= now_ts - 600.0]
    if filtered != entries:
        with open(links_path, "w", encoding="utf-8") as f:
...
def safe_send(proc, text):
    ...
    try:
        ...
        set_status("manager", "connection", "WG-MODULE-SEND", f"Could not send to module: {exc}")
...
def check_runtime_commands():
    global last_resynth_request, last_tool_update_request
    ...
    shutdown_requested_at = float(runtime_data.get("shutdown_requested_at", 0.0) or 0.0)
    if shutdown_requested_at > 0.0:
        print("Shutdown requested from UI.")
        shutdown_all()

    tool_update_requested_at = float(runtime_data.get("tool_update_requested_at", 0.0) or 0.0)
    if tool_update_requested_at > last_tool_update_request:
        last_tool_update_request = tool_update_requested_at
        state = load_update_state()
        if state.get("tool", {}).get("status") != "update_available":
            print("Tool update requested, but no update is currently available.")
            return
        if not os.path.isfile(updater_script):
            set_status("manager", "error", "WG-UPDATER-MISSING", "Updater manager file is missing.")
            print("Updater manager file is missing.")
            return
        print("Launching updater manager.")
        run_non_blocking(updater_script, capture_output=True, target_name="updater")
        shutdown_all()


def cleanup_links_loop():
    while True:
        prune_detected_links()
        time.sleep(60)
...
def integrity_monitor_loop():
    while True:
        ...
        shutdown_all(run_integrity_exit=True)


def check_tool_update_status():
    state = load_update_state()
    local_version = load_local_version()
    local_rank = parse_version_rank(local_version)
    tool_state = dict(state.get("tool", {}))
    tool_state["local_version"] = local_version
    tool_state["checked_at"] = time.time()
    try:
        zip_bytes, branch, error_message = fetch_remote_zip(TOOL_REPO)
        if not zip_bytes:
            raise RuntimeError(error_message or "Could not download the tool repo.")
        remote_version, root_name = extract_remote_version_from_zip(zip_bytes)
        if not remote_version:
            raise RuntimeError("Could not locate version.txt in the tool repo archive.")
        remote_rank = parse_version_rank(remote_version)
        if local_rank is not None and remote_rank is not None:
            status = "update_available" if remote_rank > local_rank else "up_to_date"
        else:
            status = "update_available" if remote_version and not local_version else "up_to_date"
        if local_rank is not None and remote_rank is not None and remote_rank < local_rank:
            message = f"GitHub version: {remote_version} (older than this build)"
        else:
            message = f"Remote version: {remote_version or root_name or 'Unknown'}"
        tool_state.update({
            "status": status,
            "message": message,
        ...
        if tool_state["status"] != "error":
            clear_status("github_tool")
    except Exception as exc:
        tool_state.update({
            "status": "error",
            "message": str(exc) or "GitHub check failed.",
            "remote_version": "",
            "branch": "",
        })
        set_status("github_tool", "connection", "WG-GITHUB-TOOL", tool_state["message"])
    state["tool"] = tool_state
    save_update_state(state)


def tool_update_check_loop():
    while True:
        check_tool_update_status()
        time.sleep(1800)


def update_external_logged_info():
    state = load_update_state()
    logged_state = dict(state.get("logged_info", {}))
    logged_state["checked_at"] = time.time()
    try:
        release_data = fetch_github_json(f"https://api.github.com/repos/{LOGGED_INFO_REPO}/releases/latest")
        assets = release_data.get("assets", []) if isinstance(release_data, dict) else []
        txt_asset = next((asset for asset in assets if str(asset.get("name", "")).lower().endswith(".txt")), None)
        if not txt_asset:
            raise RuntimeError("No logged player .txt asset was found in the latest release.")
        file_name = normalize_text(txt_asset.get("name", ""))
        download_url = normalize_text(txt_asset.get("browser_download_url", "")) or normalize_text(txt_asset.get("url", ""))
        if not download_url:
            raise RuntimeError("The latest logged-info release does not expose a download URL.")
        response = requests.get(
            download_url,
            timeout=20,
            headers={"User-Agent": GITHUB_HEADERS["User-Agent"], "Accept": "application/octet-stream"},
        )
        response.raise_for_status()
        file_bytes = response.content
        branch = normalize_text(release_data.get("target_commitish", ""))
        clear_status("github_logged")
    except Exception as exc:
        logged_state.update({
            "status": "error",
            "message": str(exc) or "Could not reach logged info repo.",
            "branch": "",
        })
        state["logged_info"] = logged_state
        save_update_state(state)
        set_status("github_logged", "connection", "WG-GITHUB-LOGGED", logged_state["message"])
        return

    os.makedirs(logged_external_dir, exist_ok=True)
    current_path = find_closest_log_file(logged_external_dir)
    current_bytes = b""
    ...
    updated = current_bytes != file_bytes or (current_path and os.path.basename(current_path) != file_name)
    if updated:
        for path in glob.glob(os.path.join(logged_external_dir, "logged_players_*.txt")):
            try:
                os.remove(path)
            except OSError:
                pass
        target_path = os.path.join(logged_external_dir, file_name)
        with open(target_path, "wb") as f:
            f.write(file_bytes)
        refresh_sound_caches()
        print(f"Updated external logged info: {file_name}")

    logged_state.update({
        "status": "updated" if updated else "up_to_date",
        "message": f"{'Updated' if updated else 'Already using'} {file_name}",
        "branch": branch,
        "file_name": file_name,
    })
    state["logged_info"] = logged_state
...
def external_logged_info_update_loop():
    while True:
        update_external_logged_info()
        time.sleep(3600)


def shutdown_all(run_integrity_exit=False):
    global osc_process, counter_process, ui_process

    try:
        runtime_data = load_json_safe(runtime_path, RUNTIME_DEFAULT)
        runtime_data["shutdown_requested_at"] = 0.0
        save_json(runtime_path, runtime_data)
    except Exception:
        pass

    for proc in (osc_process, counter_process, ui_process):
        if proc and is_running(proc) and proc.stdin:
            try:
                proc.stdin.write("0001\n")
                proc.stdin.flush()
            except Exception:
                pass
    ...
    if run_integrity_exit:
        try:
            ensure_project_integrity_or_exit(source="manager")
        finally:
            os._exit(1)
    os._exit(0)
...
def scan_startup_info():
    ...
    save_session_info({
        "build": normalize_text(build),
        "cpu": normalize_text(cpu),
        "ram": normalize_text(ram),
        "gpu": normalize_text(gpu),
        "vram": normalize_text(vram),
        "os": normalize_text(os_info),
        "local_user": normalize_text(local_user),
        "local_user_id": normalize_text(local_user_id),
...
def main():
    global log_path, last_resynth_request, last_tool_update_request, ui_process

    clear_status("manager")
    ...
    last_tool_update_request = float(get_runtime_dict().get("tool_update_requested_at", 0.0) or 0.0)

    log_path = resolve_log_path()
    if not log_path or not os.path.isfile(log_path):
        set_status("manager", "connection", "WG-LOG-MISSING", "No VRChat output log could be found.")
    ...
    threading.Thread(target=cleanup_links_loop, daemon=True).start()
    threading.Thread(target=integrity_monitor_loop, daemon=True).start()
    threading.Thread(target=tool_update_check_loop, daemon=True).start()
    threading.Thread(target=external_logged_info_update_loop, daemon=True).start()

    if os.path.isfile(ui_script):
        ui_process = run_non_blocking(ui_script, capture_output=True, target_name="main_ui")
    ...
    with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
        ...
        while True:
            ...
            if join_event:
                ...
                send_reason_webhooks("join", username, user_id)
                continue
            ...
            if leave_event:
                ...
                send_reason_webhooks("leave", username, user_id)
